...
int             main()
{
	...
	*/
	...
	color.g = 211;
	color.b = 13;
	put_pixel_sdl(rt_struct, 100, 100, color);
	put_pixel_sdl(rt_struct, 101, 101, color);
	put_pixel_sdl(rt_struct, 102, 102, color);
	rt_struct.is_running = 1;
	while (rt_struct.is_running) {
        SDL_UpdateTexture(rt_struct.texture, NULL, rt_struct.pixels, SCREEN_WIDTH * sizeof(Uint32));
